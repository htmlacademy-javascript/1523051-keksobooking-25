...
const getOfferRank = (offer) => {
  let rank = 0;
  if (checkboxWifi.checked && offer.offer.features.includes(checkboxWifi.value)) {
    rank += 1;
  };
  if (checkboxDishwasher.checked && offer.offer.features.includes(checkboxDishwasher.value)) {
    rank += 1;
  };
  if (checkboxParking.checked && offer.offer.features.includes(checkboxParking.value)) {
    rank += 1;
  };
  if (checkboxWasher.checked && offer.offer.features.includes(checkboxWasher.value)) {
    rank += 1;
  };
  if (checkboxElevator.checked && offer.offer.features.includes(checkboxElevator.value)) {
    rank += 1;
  };
  if (checkboxConditioner.checked && offer.offer.features.includes(checkboxConditioner.value)) {
    rank += 1;
  };
  return rank;
};
